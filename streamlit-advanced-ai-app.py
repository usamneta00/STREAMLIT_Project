...
STREAMLIT_PASSWORD = os.getenv("STREAMLIT_PASSWORD", "yemensoft")
...
if not st.session_state.authenticated:
    ...
    if password.strip() == STREAMLIT_PASSWORD:
...
def load_models_and_data():
    ...
    # Path to the model saved from the training notebook
    local_model_path = os.path.join("backend", "models", "logistic_model_outputs.pkl")
    
    if not os.path.exists(local_model_path):
        st.error(f"Missing model artifacts. Could not find 'logistic_model_outputs.pkl' at {local_model_path}. Please run backend/train.py first.")
        st.stop()

    print(f"✅ Loading model from: {local_model_path}")
    model_artefacts = joblib.load(local_model_path)
    ...
    df_locations = pd.read_csv("backend/data/locations.csv")
    ...
    df_products = pd.read_csv("backend/data/products.csv")
